 ...
 */
export const pick = <T extends object, K extends keyof T>(obj: Readonly<T>, keys: readonly K[]): Pick<T, K> => {
 ...
 */
export const calculateDifferences = <T extends object>(oldObj: Partial<T>, newObj: T): Partial<T> => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return transform(newObj as any, (result: any, value: unknown, key: string) => {
 ...
 */
export const updateArrayElementsBy = <T extends object>(data: T[], objUpd: Partial<T>, objSearch: Partial<T>): T[] => {
 ...
 */
export const updateArrayElementById = <T extends object>(data: T[], objUpd: Partial<T>, idField: keyof T): T[] => {
...
export const flatten = <T, D extends number = 1>(
  arr: T[],
  depth?: D
): FlatArray<T[], D>[] => arr.flat(depth as D)
...
export const mapValues = <K extends string, T, U>(
  ...
  Object.fromEntries(
    (Object.entries(obj) as [K, T][]).map(([k, v]) => [k, fn(v, k)])
  ) as Record<K, U>
...
export const filterValues = <K extends string, T>(
  ...
  Object.fromEntries(
    (Object.entries(obj) as [K, T][]).filter(([k, v]) => fn(v, k))
...
export const sortBy = <T>(
  arr: T[],
  ...iteratees: Array<keyof T | ((item: T) => unknown)>
): T[] =>
  [...arr].sort((a, b) => {
