...
- Añade primitivas de resiliencia async: `retry` con exponential backoff y
  jitter, `withTimeout`, `combineAbortSignals`, `memoize` con TTL y LRU,
  `Deferred<T>`, `Semaphore` y `Mutex`.
