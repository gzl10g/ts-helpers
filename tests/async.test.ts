...
import { sleep, wait, runBatch, handleOperation, Deferred, withTimeout, combineAbortSignals, retry, memoize, Semaphore, Mutex } from '../src/async'
...
describe('combineAbortSignals', () => {
  ...
  test('abortar la primera signal → combined aborta; la segunda no dispara listeners (sin leak)', () => {
    ...
    combined.addEventListener('abort', () => { extraAbortCount++ })
    c2.abort('segunda')
