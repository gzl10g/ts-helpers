...
describe('pick', () => {
  ...
  test('objeto vacío devuelve objeto vacío', () => {
    const result = pick({} as { a?: number }, ['a' as 'a'])
    expect(result).toEqual({})
  })
...
describe('uniqBy', () => {
  test('deduplicates by key property (first occurrence wins)', () => {
    ...
    expect(uniqBy(items, 'id')).toEqual([{ id: 1, v: 'a' }, { id: 2, v: 'b' }])
  })
...
describe('sortBy', () => {
  ...
  test('sorts by multiple keys (second key breaks ties)', () => {
    const input = [{ a: 1, b: 2 }, { a: 1, b: 1 }, { a: 0, b: 9 }]
    expect(sortBy(input, 'a', 'b')).toEqual([{ a: 0, b: 9 }, { a: 1, b: 1 }, { a: 1, b: 2 }])
  })
