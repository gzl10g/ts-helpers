...
describe('URL and HTML Utilities', () => {
  ...
  test('toUrlSlug should transliterate accented characters', () => {
    expect(toUrlSlug('café')).toBe('cafe')
    expect(toUrlSlug('ñoño')).toBe('nono')
    expect(toUrlSlug('¡Café con Ñ!')).toBe('cafe-con-n')
    expect(toUrlSlug('Café Münchën 2024')).toBe('cafe-munchen-2024')
    expect(toUrlSlug('résumé')).toBe('resume')
    expect(toUrlSlug('naïve')).toBe('naive')
  })
